...
export const sendLoginOtpEmail = async ({ to, name = 'Team Member', otp, minutes = 15 }) => {
  ...
  const bodyHtml = `
    ...
    <h2 class="h1">Hello, ${escapeHtml(name)}</h2>
...
export const sendPasswordResetEmail = async ({ to, name = 'Team Member', otp, minutes = 15 }) => {
  ...
  const bodyHtml = `
    ...
    <p class="p">Hello <strong>${escapeHtml(name)}</strong>,</p>
    <p class="p">We received a request to reset the password for your account associated with <strong>${escapeHtml(to)}</strong>.</p>
...
export const sendNewUserWelcomeEmail = async ({ to, name, role = 'Member', appUrl = 'http://localhost:3001' }) => {
  ...
  const bodyHtml = `
    ...
    <h2 class="h1">Welcome aboard, ${escapeHtml(name)}!</h2>
    <p class="p">Your Shoolin OS workspace account has been created by your administrator with the <strong>${escapeHtml(role)}</strong> role.</p>

    <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:16px;margin:20px 0;">
      <p style="margin:0 0 8px 0;font-size:13px;"><strong>Login Email:</strong> ${escapeHtml(to)}</p>
      <p style="margin:0 0 8px 0;font-size:13px;"><strong>Assigned Role:</strong> ${escapeHtml(role)}</p>
      <p style="margin:0;font-size:13px;"><strong>Workstation URL:</strong> <a href="${escapeHtml(appUrl)}" style="color:#059669;">${escapeHtml(appUrl)}</a></p>
    </div>
    ...
    <ol style="font-size:13px;color:#475569;line-height:1.8;padding-left:20px;margin:0 0 24px 0;">
      <li>Navigate to the login screen at <a href="${escapeHtml(appUrl)}" style="color:#059669;">${escapeHtml(appUrl)}</a></li>
      <li>Click the <strong>"Email OTP"</strong> tab, enter your email (<strong>${escapeHtml(to)}</strong>), and receive an instant 6-digit login passcode, OR</li>
